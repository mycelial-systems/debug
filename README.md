...
Call `.error(...)` to log with the label `ERROR` in red. The enablement
rule is the same as the parent debugger — `.error` emits only when
`DEBUG` matches the namespace passed to `Debug()`.
...
log('hello')          // auth hello
log.error('oh no')    // ERROR oh no  (in red)
```

The displayed label is always `ERROR`; the parent namespace is used
only for gating, not for the output. Works the same in Node and
Cloudflare.
