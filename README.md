...
Plus,
[see a live demo of use in the browser with `localStorage`](https://substrate-system.github.io/debug/).
...
Import then create a namespaced `debug` instance.
...
The `debug` function has a property `.error`. It will log things prefixed with
`ERROR` in a red color. It will log/not log with the same logic as the parent
`debug` instance.
...
// logs only if localStorage.DEBUG includes `my-namespace`
debug.error('oh no')
```
